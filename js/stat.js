...
(function () {
  ...
  var widthPrintArea =
    background.width -
    background.scroll.spool.ledge * 2 -
    background.scroll.paper.width * 2 -
    background.shadowOffset;
  var heightPrintArea =
    background.height -
  ...
  window.renderStatistics = function (ctx, names, times) {
    // ширина катушки;
    var spoolWidth =
      background.scroll.paper.width + background.scroll.spool.ledge * 2;

    //  заливка для левой катушки свитка
    background.scroll.spool.leftСolor = ctx.createLinearGradient(
        background.initialX,
        background.initialY,
    ...
    background.scroll.spool.leftСolor.addColorStop(
        0.55,
        'rgba(125, 45, 16, 1)'
    );
    ...
    background.scroll.spool.rightСolor = ctx.createLinearGradient(
    ...
    background.scroll.spool.rightСolor.addColorStop(
        0.4,
        'rgba(140, 52, 19, 1)'
    );
    background.scroll.spool.rightСolor.addColorStop(
    ...
    background.scroll.paper.leftСolor = ctx.createLinearGradient(
    ...
    background.scroll.paper.leftСolor.addColorStop(
        0.6,
        'rgba(255, 255, 255, 1)'
    );
    background.scroll.paper.leftСolor.addColorStop(
    ...
    background.scroll.paper.rightСolor = ctx.createLinearGradient(
        printAreaX + widthPrintArea,
        background.initialY,
        printAreaX + widthPrintArea + background.scroll.paper.width,
        background.initialY
    );
    background.scroll.paper.rightСolor.addColorStop(
        0,
        'rgba(127, 127, 127, 1)'
    );
    background.scroll.paper.rightСolor.addColorStop(
    ...
    //  тень от свитка
    background.scrollShadow = ctx.createLinearGradient(
        printAreaX,
        background.initialY,
        printAreaX + widthPrintArea,
        background.initialY
    ...
    ctx.fillText(
        title.firstLineText,
        title.initialX,
        title.initialY + title.lineHeight
    );
    ctx.fillText(
        title.secondLineText,
        title.initialX,
        title.initialY + title.lineHeight * 2
    );
    ...
    var barSpace =
      widthPrintArea /
      (diagram.indentRatio * 2 + barSum * diagram.widthRatio + barSum - 1);
    ...
    for (var i = 0; i < names.length; i += 1) {
      ...
      colorOfBar =
        names[i] === 'Вы' ? diagram.mainColor : diagram.secondaryColor;
